...
const iconMap: Record<string, React.ElementType> = { Image, Anchor, Building2, Users };

export default function OasisPage() {
  const { data: pageData } = usePageData("oasis");

  const categories: Array<{ icon: string; title: string; description: string }> = pageData?.categories || [];
  ...
  return (
    ...
    >
      <div className="space-y-16">
        ...
        {categories.length > 0 && (
          <div>
            <h2 className="text-2xl font-bold text-foreground mb-8">{pageData?.categoriesTitle || "Categorias de Dados"}</h2>
            <div className="grid md:grid-cols-2 gap-6">
              {categories.map((item, i) => {
                const Icon = iconMap[item.icon] || Image;
                return (
